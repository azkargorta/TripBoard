...
function buildPlanPlaces(rows: unknown[] | undefined, prefix: string): PlaceOption[] {
  return (rows ?? []).map((row, index) => {
    ...
    const label =
      asString(item.title) ?? asString(item.name) ?? asString(item.place_name) ?? asString(item.location_name) ?? `Lugar ${index + 1}`;
...
function buildInitialRoutes(rows: unknown[] | undefined, prefix: string): TripMapRoute[] {
  return (rows ?? []).map((row, index) => {
    ...
    const destinationLat = asNumber(item.destination_latitude) ?? asNumber(item.end_latitude) ?? asNumber(item.to_latitude);
    const destinationLng = asNumber(item.destination_longitude) ?? asNumber(item.end_longitude) ?? asNumber(item.to_longitude);
...
export default function TripMapView({ tripId, tripDates = [], planSources, routeSources }: Props) {
  ...
  useEffect(() => setRoutesState(initialRoutes), [initialRoutes]);
  useEffect(() => { if (hookRouteError) setRouteError(hookRouteError); }, [hookRouteError]);
  ...
  const visiblePoints = useMemo(() => {
    const pts = planPlaces
      .filter((place) => selectedDate === "all" || place.activityDate === selectedDate)
      .map((place) => ({ id: place.id, latitude: place.latitude, longitude: place.longitude, title: place.label }));
    return highlightedRouteId ? [] : pts;
  ...
  useEffect(() => {
    ...
    async function loadDirections() {
      ...
      entries.forEach(([id, result]) => { next[id] = result; });
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => fitMapToData(), 150);
  ...
  const applyPlaceSelection = (setState: React.Dispatch<React.SetStateAction<FormPlaceState>>, placeId: string) => {
  ...
  const buildGoogleMapsUrl = (route: TripMapRoute) => {
    ...
    if (route.stop_address || route.stop_name) url += `&waypoints=${encodeURIComponent(route.stop_address || route.stop_name || "")}`;
    return url;
  };
  ...
  const calculateRoute = async () => {
    ...
    if (typeof origin.latitude !== "number" || typeof origin.longitude !== "number" || typeof destination.latitude !== "number" || typeof destination.longitude !== "number") {
      return setRouteError("Origen y destino deben tener coordenadas válidas.");
    }
    ...
    try {
      ...
      const result = await service.route({
        ...
        waypoints: hasStop && typeof stop.latitude === "number" && typeof stop.longitude === "number"
          ? [{ location: { lat: stop.latitude, lng: stop.longitude }, stopover: true }]
          : [],
        travelMode: gm.maps.TravelMode[normalizeTravelMode(travelMode)],
        provideRouteAlternatives: false,
      });
      ...
      setPreview({
        ...
        durationText,
        arrivalTime,
        overviewPath: result.routes[0]?.overview_path?.map((point) => ({ lat: point.lat(), lng: point.lng() })) || [],
      });
    } catch (error) {
      console.error(error);
      ...
      const saved = await saveRoute(
        {
          ...
          stops: hasStop && typeof stop.latitude === "number" && typeof stop.longitude === "number"
            ? [{ name: stop.address, address: stop.address, latitude: stop.latitude, longitude: stop.longitude }]
            : [],
  ...
  const startEdit = (route: TripMapRoute) => {
    setIsCreating(true);
    ...
    setOrigin({ mode: "search", planId: "", address: route.origin_address || route.origin_name || "", latitude: route.origin_latitude ?? null, longitude: route.origin_longitude ?? null });
    setHasStop(route.stop_latitude != null && route.stop_longitude != null);
    setStop({ mode: "search", planId: "", address: route.stop_address || route.stop_name || "", latitude: route.stop_latitude ?? null, longitude: route.stop_longitude ?? null });
    setDestination({ mode: "search", planId: "", address: route.destination_address || route.destination_name || "", latitude: route.destination_latitude ?? null, longitude: route.destination_longitude ?? null });
    setPreview(null);
  };
  ...
  if (loadError) {
    return <div className="rounded-2xl border border-red-200 bg-red-50 p-6 text-sm text-red-700">No se pudo cargar Google Maps. Revisa la API key, Maps JavaScript API, Places API y Directions API.</div>;
  }
    ...
    <div className="grid gap-6 lg:grid-cols-[420px_minmax(0,1fr)]">
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            {dateOptions.map((date) => (
              <button
                ...
                className={`rounded-full px-3 py-1.5 text-sm font-medium transition ${selectedDate === date ? "bg-slate-900 text-white" : "border border-slate-200 bg-white text-slate-700"}`}
              >
        ...
        {isCreating ? (
          <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
            <div className="flex items-center justify-between gap-3">
              <h2 className="text-lg font-bold text-slate-950">{editingRouteId ? "Editar ruta" : "Nueva ruta"}</h2>
              {editingRouteId ? (
                <button type="button" onClick={() => { setIsCreating(false); resetForm(); }} className="text-sm text-slate-500 hover:text-slate-700">
                  Cancelar edición
                </button>
              ) : null}
            </div>
            ...
            <div className="mt-4 space-y-4">
              <div className="grid gap-4 md:grid-cols-2">
                <label className="block space-y-2">
                  ...
                  <input type="date" value={routeDate} onChange={(e) => setRouteDate(e.target.value)} className="w-full rounded-xl border border-slate-300 bg-white px-4 py-3" />
                </label>
                <label className="block space-y-2">
                  <span className="text-sm font-semibold text-slate-800">Hora de salida</span>
                  <input type="time" value={departureTime} onChange={(e) => setDepartureTime(e.target.value)} className="w-full rounded-xl border border-slate-300 bg-white px-4 py-3" />
                </label>
              </div>

              <label className="block space-y-2">
                <span className="text-sm font-semibold text-slate-800">Nombre de la ruta</span>
                <input type="text" value={routeName} onChange={(e) => setRouteName(e.target.value)} placeholder="Ej. Hotel → Playa" className="w-full rounded-xl border border-slate-300 bg-white px-4 py-3" />
              </label>

              {[{ label: "Origen", state: origin, setState: setOrigin }, { label: "Destino", state: destination, setState: setDestination }].map((item) => (
                <div key={item.label} className="space-y-3 rounded-2xl border border-slate-200 p-4">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex rounded-xl border border-slate-200 bg-slate-50 p-1 text-xs">
                      <button type="button" onClick={() => item.setState((prev) => ({ ...prev, mode: "plan" }))} className={`rounded-lg px-3 py-1 ${item.state.mode === "plan" ? "bg-white shadow-sm" : "text-slate-500"}`}>Desde Plan</button>
                      <button type="button" onClick={() => item.setState((prev) => ({ ...prev, mode: "search", planId: "" }))} className={`rounded-lg px-3 py-1 ${item.state.mode === "search" ? "bg-white shadow-sm" : "text-slate-500"}`}>Buscar</button>
                    </div>
                  </div>

                  {item.state.mode === "plan" ? (
                    <select value={item.state.planId} onChange={(e) => applyPlaceSelection(item.setState, e.target.value)} className="w-full rounded-xl border border-slate-300 bg-white px-4 py-3">
                      <option value="">Selecciona un lugar del plan</option>
                      {planDateOptions.map((place) => (
                        <option key={place.id} value={place.id}>{place.label}{place.activityDate ? ` · ${place.activityDate}` : ""}</option>
                      ))}
                    </select>
                  ) : (
                    <PlaceAutocompleteInput
                      ...
                      onPlaceSelect={(payload) => item.setState((prev) => ({ ...prev, address: payload.address, latitude: payload.latitude, longitude: payload.longitude }))}
                    />
                  )}
                </div>
              ))}
              ...
              <div className="rounded-2xl border border-slate-200 p-4">
                <label className="flex items-center gap-3">
                  <input type="checkbox" checked={hasStop} onChange={(e) => setHasStop(e.target.checked)} />
                ...
                {hasStop ? (
                  <div className="mt-4 space-y-3">
                    <div className="flex rounded-xl border border-slate-200 bg-slate-50 p-1 text-xs w-fit">
                      <button type="button" onClick={() => setStop((prev) => ({ ...prev, mode: "plan" }))} className={`rounded-lg px-3 py-1 ${stop.mode === "plan" ? "bg-white shadow-sm" : "text-slate-500"}`}>Desde Plan</button>
                      <button type="button" onClick={() => setStop((prev) => ({ ...prev, mode: "search", planId: "" }))} className={`rounded-lg px-3 py-1 ${stop.mode === "search" ? "bg-white shadow-sm" : "text-slate-500"}`}>Buscar</button>
                    </div>
                    {stop.mode === "plan" ? (
                      <select value={stop.planId} onChange={(e) => applyPlaceSelection(setStop, e.target.value)} className="w-full rounded-xl border border-slate-300 bg-white px-4 py-3">
                      ...
                      <PlaceAutocompleteInput
                        ...
                        onPlaceSelect={(payload) => setStop((prev) => ({ ...prev, address: payload.address, latitude: payload.latitude, longitude: payload.longitude }))}
                      />
                    )}
                  </div>
                ) : null}
              ...
              <label className="block space-y-2">
                ...
                <select value={travelMode} onChange={(e) => setTravelMode(e.target.value as RouteMode)} className="w-full rounded-xl border border-slate-300 bg-white px-4 py-3">
              ...
              <div className="flex flex-wrap gap-3">
                <button type="button" onClick={calculateRoute} disabled={calculating} className="rounded-2xl border border-slate-300 bg-white px-4 py-3 text-sm font-semibold text-slate-900 transition hover:bg-slate-50 disabled:opacity-50">
                  {calculating ? "Calculando..." : "Calcular ruta"}
                </button>
                <button type="button" onClick={handleSave} disabled={!preview || savingRoute} className="rounded-2xl bg-violet-600 px-4 py-3 text-sm font-semibold text-white transition hover:bg-violet-700 disabled:opacity-50">
        ...
        <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <div className="mt-4 space-y-3">
            {visibleRoutes.length === 0 ? (
              <div className="rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-4 text-sm text-slate-600">No hay rutas para este día.</div>
            ) : (
                ...
                <article key={route.id} className="rounded-2xl border border-slate-200 p-4">
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      ...
                      <p className="mt-1 text-sm text-slate-600">{(route.route_day || route.route_date || "Sin día")}{route.departure_time ? ` · ${route.departure_time}` : ""}</p>
                      <p className="mt-1 text-sm text-slate-600">{(route.origin_name || route.origin_address || "Origen")} → {(route.destination_name || route.destination_address || "Destino")}</p>
                      {route.distance_text || route.duration_text ? <p className="mt-1 text-sm text-slate-500">{[route.distance_text, route.duration_text].filter(Boolean).join(" · ")}</p> : null}
                    </div>
                    <span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-semibold text-slate-700">{normalizeTravelMode(route.travel_mode)}</span>
                  </div>

                  <div className="mt-4 flex flex-wrap gap-2">
                    <button type="button" onClick={() => setHighlightedRouteId((prev) => (prev === route.id ? null : route.id))} className="rounded-xl border border-slate-300 bg-white px-3 py-2 text-sm font-medium text-slate-800 hover:bg-slate-50">
                      {highlightedRouteId === route.id ? "Mostrar todas" : "Mostrar ruta"}
                    </button>
                    <a href={buildGoogleMapsUrl(route)} target="_blank" rel="noreferrer" className="rounded-xl border border-slate-300 bg-white px-3 py-2 text-sm font-medium text-slate-800 hover:bg-slate-50">
                      Abrir en Google Maps
                    </a>
                    <button type="button" onClick={() => startEdit(route)} className="rounded-xl border border-slate-300 bg-white px-3 py-2 text-sm font-medium text-slate-800 hover:bg-slate-50">
                      Editar ruta
                    </button>
                    <button type="button" onClick={() => void handleDelete(route.id)} className="rounded-xl border border-red-200 bg-red-50 px-3 py-2 text-sm font-medium text-red-700 hover:bg-red-100">
                      Eliminar ruta
                    </button>
                  </div>
                </article>
      ...
      <div className="space-y-4">
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-4 text-sm text-slate-600">
          {highlightedRouteId ? "Mostrando solo la ruta seleccionada en el mapa." : selectedDate === "all" ? "Mostrando rutas de todos los días." : `Mostrando rutas del día ${selectedDate}.`}
        </div>
      </div>
    </div>
  );
