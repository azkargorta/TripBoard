...
export default function DesktopTripSidebar({ tripId, isPremium }: Props) {
  ...
  const visibleItems = isPremium ? items : items.filter((i) => i.key !== "map" && i.key !== "chat");

  return (
    <aside className="hidden md:block">
      <div className="sticky top-24">
        <div className="rounded-3xl border border-slate-200 bg-white/85 p-3 backdrop-blur supports-[backdrop-filter]:bg-white/70">
            ...
            {visibleItems.map((item) => {
              ...
              return (
                <Link
                  ...
                  href={href}
                  prefetch
                  className={`flex min-h-[44px] items-center gap-3 rounded-2xl px-3 py-2 text-sm font-semibold transition ${
                    active
