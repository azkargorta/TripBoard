...
export async function generateExecutableItineraryFromIntent(
  ...
  const generateDays = Math.max(1, Math.min(resolved.durationDays, cfg.autoGenerateDays));
...
Fechas por día:
${dayLines.join("\n")}
Duración total del viaje: ${resolvedForPrompt.durationDays} días. Genera planes detallados SOLO para los primeros ${generateDays} días.
